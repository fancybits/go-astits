...
func pesWithHeaderBytes() []byte {
	...
	buf.Write(make([]byte, 200)) // Add some extra bytes
	return buf.Bytes()
}
...
func TestParsePESData(t *testing.T) {
	for _, tc := range pesTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			buf.Write(make([]byte, 200)) // Add some extra bytes
			d, err := parsePESData(astikit.NewBytesIterator(buf.Bytes()), false)
			assert.NoError(t, err)
...
func BenchmarkParsePESData(b *testing.B) {
	...
	for ti, tc := range pesTestCases {
		b.Run(tc.name, func(b *testing.B) {
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				parsePESData(astikit.NewBytesIterator(bss[ti]), false)
			}
		})
	}
}
